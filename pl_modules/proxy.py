...
class M3gnetDGL_Proxy():
    ...
    def __call__(self, m):
        small_crystal_mask = np.asarray([(len(i.structure.species)<100) and (i.structure.density > 1.5) and (i.structure.density < 5.0) for i in m])
        ...
        preds[preds < -5] = 10
        preds = np.nan_to_num(preds,nan=100.0)
        return preds